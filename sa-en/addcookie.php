...
if(isset($_GET['getcart'])) {
	...
	if($_SESSION['restaurantid']== true && $_SESSION['randID']==true) 
	{
		...
		$day_db = array();
		if($_SESSION['booking_available'])
		{
			$data 		= array(
				'restaurant_id' =>	$restaurantid
			);
			$endpoint 	= "/getRestaurantBookingDays";
			$day_db 	= curl_request($data, $endpoint, $baseurl);
		...
		if($responseCount!="0")
		{
							...
							<li>
								<div class="topbotpadr padr" style="padding:10px 10px;">
									<h2 class="title" style="font-size:15px; ">
										<div class="left col70" style="font-weight: bold;">
											<?php 
												echo "<span style=\"color: #00C269;\">".$value['menu_item_quantity']." x "."</span>";
												echo $value['menu_item_name'];
											?>	
										</div>
										<div class="right extraitemprice" style="font-weight: bold; color: #00C269;">
											<?php 
			...
			?>
				...
				<div class="couponcode delivery_row" style="background: white;">
					<div class="left col50" id="delivery" onClick="orderType('delivery');">Delivery</div>
					<div class="right col50" id="pickup" onClick="orderType('pickup');" >Pickup</div>
					<div class="clear"></div>
					<input type="hidden" id="booking_day_id" value="" name="booking_day_id">
					<input type="hidden" id="booking_date" value="" name="booking_date">
					<input type="hidden" name="booking_day_time_id" id="booking_day_time_id" value="">
					<input type="hidden" name="booking_time_id" id="booking_time_id" value="">
					<input type="hidden" id="booking_available_status" name="booking_available_status" value="<?php echo $_SESSION['booking_available'];?>">
					<div id="booking_available" style="font-size: 14px;margin-left: -35px;width: 131%; display: none" >
					
						<ul class="time_slot couponcode" id="time_slot" style="width: 100%;">
					<?php
					
						if(!empty($day_db['msg']))
						{

						foreach($day_db['msg'] as $head){
					?>
				
						<li class="day" data-day="<?php echo strtolower($head['booking_day']);?>" id="<?php echo strtolower($head['booking_day']);?>" onclick="daySelect('<?php echo strtolower($head['booking_day']);?>', '<?php echo strtolower($head['booking_date']);?>')"><?php echo ucwords($head['booking_day']);?></li>
                    <?php
						}
					?>
					 	</ul>
					...
					<div id="time_slot_<?php echo strtolower($head['booking_day']);?>" style="display:none">
                            
					<input type="hidden" name="booking_time_id_<?php echo strtolower($head['booking_day']);?>" value="<?php echo $head['booking_time_id']?>" id="booking_time_id_<?php echo strtolower($head['booking_day']);?>">
						 <ul class="time_slot_day" style="margin-left: -34px; width: 111%;">
					
				
						<?php 
							//foreach($head['time']['BookingTime'] as $key => $time){
							foreach($head['time'] as $time){
						?>
							<li class="day_time" id="day_time_<?php echo strtolower($time['BookingTime']['id']);?>_<?php echo strtolower($head['booking_day']);?>" onclick="dayTmeSelect('<?php echo strtolower($time['BookingTime']['id']);?>', '<?php echo strtolower($head['booking_day']);?>')"><?php echo $time['BookingTime']['booking_time'];?></li>
							<?php 
							}
							?>
					 	</ul>
				...
				<div class="couponcode">
					<input class="left" type="text" id="couponcode" name="couponcode" placeholder="Coupon Code" />
					<span  class="right col20" id="cupnbtn">
						<button type="button" onclick="verifyCoupan()">Verify</button>
					</span>	
					<div class="clear"></div>
				...
				<div id="address_cart_container" style="display: none; width: 100%; text-align: center; margin-top: 5%; margin-bottom: 5%; font-family: Montserrat; font-size: 13px;">
					...
					<span onClick="popup('ordernow')" style="font-size: 10px; font-weight: 100; cursor: pointer; color: #be2c2c;">Modify</span>
				</div>
			</div>
				<div class="couponcode totalBox">
					<div class="total_row" style="display: none">
						<div class="left col60" >Rider Tip</div>
						<div class="right col40 textright"><?php echo $symbol; ?><span id="checkoutridertip">0.00</span></div>
						<div class="clear"></div>
					</div>
					<div class="total_row">
						<div class="left col60" >SubTotal</div>
						<input type="hidden" id="subtotal" value="<?php echo $subtotal; ?>">
						<div class="right col40 textright"><span id="subtotalinvoice"><?php echo $subtotal; ?>.00</span><?php echo ' '.$symbol; ?></div>
						<div class="clear"></div>
					</div>
					<div class="total_row">
						<div class="left col60" >Tax(<?php echo $tax_persent; ?>)</div>
						<div class="right col40 textright"><?php echo $tax_percentage.' '; echo $symbol; ?></div>
						<input type="hidden" id="tax" value="<?php echo $tax_percentage; ?>">
						<div class="clear"></div>
					</div>
					<div class="total_row">
						<div class="left col60" >Discount</div>
						<div class="right col40 textright"><span id="discountinvoice">0.00</span><?php echo ' '.$symbol; ?></div>
						<div class="clear"></div>
						<input type="hidden" id="discountvalue" value="0">
					</div>
					<div class="total_row">
						<div class="left col60" >Delivery Fee</div>
						<div class="right col40 textright"><span id="deliveryfeeinvoice">0.00</span><?php echo ' '.$symbol; ?></div>
						<input type="hidden" id="delivery_fee" value="0">
						<div class="clear"></div>
					</div>
					<hr>
					<div class="total_row">
						<div class="left col60" >Total</div>
						<div class="right col40 textright" style="color: #00C269"><span id="total_price" style="color: #00C269"><?php echo $subtotal+$tax_percentage; ?></span><?php echo ' '.$symbol; ?></div>
				...
				<div class="cartbelow">
					<a <?php if($_SESSION['id']!=""){echo 'onClick="orderNow()"';} else {echo 'onClick="popup(\'login\')"';} ?> id="ordernowbutton_" name="ordernowbutton_"><button type="button" class="button">Order Now</button></a>
				</div>
		...
		<form id="addtocart_popup" name="addtocart_popup" action="#" method="post">
				...
				<?php 
					...
					if($sectionCount!=0)
					{
						foreach ($value3['RestaurantMenuExtraSection'] as $key4 => $value4) {
							...
							?>
							<li>
								<div class="topbotpadr padr">
									<h2 class="title"><?php echo ucwords($value4['name']); ?></h2>
									<?php if( $req == 1 ) { ?>
									<div style="margin-top: 5px;font-size: 16px;">
										<div class="left col80">
											<p><span style='color:#aaa;display:inline-block;'>Choose One</span></p>
										</div>
										<div class="right">
											<p><span style='color:red;display:inline-block;'>Required</span></p>
										</div>
										<div class="clear"></div>
									</div>
									<?php } ?>
    										...
    										<li>
    											<?php if( $req == 1 ) { ?>
    												<div class="radio">
    													...
    													<label class="lable-radio-container">
    											...
    											<?php } else { ?>
    												<div class="checkbox">
    													...
    													<label class="lable-container extraitem_req_<?php echo $value5['id']; ?>" >
    														...
    														<span class="checkmark"></span>
    														<div class="left col70"><?php echo $value5['name']; ?></div>
    														<div class="right extraitemprice" style="color: #00C269">
    																<?php
			...
			<div class="submitbtnn" style="background: white;">
				...
				<button type="button" class="submitbtnn" id="popupAddtocartbtn" onClick="return addtocart(addtocart_popup)" style="background: white; ">
					<span class="center" style="background: #E86942; padding: 13px; border-radius: 25px; width: 20%; margin: 0 0 0 10%;">Add to Cart</span>
					<div class="right extraitemprice" style="color: #00C269; font-weight: bold;">
...
if(isset($_GET['placeorder'])) 
{
	...
	$endpoint = "/placeorder";
		...
		$err = curl_error($curl);
         ...
         //echo $json_data['msg'];
		 echo "<div style='text-align: center;font-size: 20px;padding: 40px 0 20px 0;'><img src='assets/img/nocart.png' alt='place order' style='width: 180px; margin-top:60px;''> <br/><div style='margin: 20px 0 0 0; padding: 0; font-size: 20px; font-weight: 300;'>Your order has been placed successfully. your order number is ".$json_data['msg'][0]['Order']['order_number']."</div>";
