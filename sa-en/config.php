...
define("FACEBOOK_LOGIN_ENABLE","true");   // value should be true of false
//enable/disable google login
define("GOOGLE_LOGIN_ENABLE","true");   // value should be true of false

//enable/disable phone login
define("PHONE_LOGIN_ENABLE","true");   // value should be true of false
...
//echo "<pre>";print_r($_POST);die;
	...
	$phone_code = $_POST['phne_code'];
    $phone = $phone_code.$phone1;
	...
	if( $flag ) { 
	   ...
	   // var_dump($json_data);die;
		...
		if($json_data['code'] !== 200)
		{
			...
			$name = explode(" ", $json_data['msg']['UserInfo']['full_name']);

			$_SESSION['sessionTokon'] = time();
			$_SESSION['id'] = $json_data['msg']['UserInfo']['user_id'];
			$_SESSION['first_name'] = $name[0];
			$_SESSION['last_name'] = $name[1];
			$_SESSION['name'] = $json_data['msg']['UserInfo']['full_name'];
			$_SESSION['phone'] = $json_data['msg']['UserInfo']['phone'];
			$_SESSION['device_token'] = $json_data['msg']['UserInfo']['device_token'];
			$_SESSION['online'] = $json_data['msg']['UserInfo']['online'];
			$_SESSION['email'] = $json_data['msg']['User']['email'];
