...
class ConfigController extends AppController
{
    ...
    public function test(){
        ...
        // $testPostMark = CustomEmail::testPostMarknew();
        // pr($testPostMark);
        ...
        $this->autoRender = false;
            ...
            if (is_array($test_stripe)) {
                if (array_key_exists('error', $test_stripe)) {
                    ...
                    $data['STRIPE_API_KEY'] = 0;
                    $data['STRIPE_FAILED'] = $test_stripe['error']['message'];
                } else {
            ...
            //             $data['POSTMARK_SERVER_API_TOKEN'] = 0;
            //             $data['POSTMARK_FAILED'] = $testPostMark['Message'];
           ...
           /*reasons failed status*/

           if($data['GOOGLE_MAPS_GEOCODE'] < 1){

               $data['GOOGLE_MAPS_FAILED'] = $results['output'];

           }

            if($data['TWILIO'] < 1){

                $data['TWILIO_FAILED'] = $response['message'];
