...
//             $json_data = @curl_request($data, $url);
            ...
            if ($json_code !== 200) {
                echo "<script>window.location='dashboard.php?p=editResturent&id=".$restaurant_id."&action=error'</script>";
            } else {
                echo "<script>window.location='dashboard.php?p=editResturent&id=".$restaurant_id."&action=success'</script>";
            }
        ...
        if($_GET['action'] == "deletecoupon" && !empty($_GET['coupon_id'])) {
        		...
        		if($json_data['code'] !== 200){
        			...
        			@header("Location: dashboard.php?p=editResturent&id=".$user_id."&action=error");
        				echo "<script>window.location='dashboard.php?p=editResturent&id=".$user_id."&action=error'</script>";
        
        		} else {
        			//echo "<div class='alert alert-success'>Successfully coupon code added..</div>";
        			@header("Location: dashboard.php?p=editResturent&id=".$user_id."&action=success");
        				echo "<script>window.location='dashboard.php?p=editResturent&id=".$user_id."&action=success'</script>";
        		}
...
<?php
    ...
    <div class="main-container dataTables_wrapper" id="table_view_wrapper" >
        ...
        <div>
            ...
            <!--<form id="catagoryform" action="dashboard.php?p=editResturent&id=<?php echo $id ?>&action=addRestaurantcatagoryImage" method="post" enctype="multipart/form-data">
            <div class="qr-el qr-el-3" style="min-height: auto; float:left; box-shadow: 2px 0px 30px 5px rgba(0, 0, 0, 0.03);">
            ...
            <form id="logoform" accept-charset="utf-8" action="dashboard.php?p=editResturent&id=<?php echo $id ?>&action=addtoresturentdata" method="post" enctype="multipart/form-data">
        ...
        <h3 style="font-weight: 300;" align="center">Booking Availablity Information</h3>
            ...
            <form action="dashboard.php?p=editResturent&id=<?php echo $id ?>&action=updateDeliveryTime" method="post">
				...
				<div class="cl3 left col30" style="margin-right: 10px; width: 30%; heigth: 50px; display: inline-block;">
					...
					<div class="itm" style="background: #F9F9F9; padding: 15px;	border-radius: 3px;	margin-bottom: 10px;	position: relative;	line-height: 1.5em;	border: 1px solid #ddd; font-size: 14px;"> 
						...
						<div style="color: red;text-align: right;font-size: 13px;">
						    <a href="dashboard.php?p=editResturent&action=deletecoupon&coupon_id=<?php echo $vaaal['RestaurantCoupon']['id']; ?>&id=<?php echo $user_id; ?>">
